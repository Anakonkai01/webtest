...
def get_all_phones_route():
    ...
    if paginated_phones.has_next:
        ...
        next_url = url_for(request.endpoint, _external=False, **next_url_params)
    if paginated_phones.has_prev:
        prev_url_params = current_args.copy()
        prev_url_params['page'] = paginated_phones.prev_num
        prev_url = url_for(request.endpoint, _external=False, **prev_url_params)
...
def update_phone_route(phone_id):
    ...
    if not validated_data:
         abort(400, description="Không có trường hợp lệ nào được cung cấp để cập nhật hoặc dữ liệu không hợp lệ.")
    ...
    if updated_fields_count > 0:
        ...
        return jsonify(message="Không có thay đổi nào được thực hiện hoặc dữ liệu giống hệt hiện tại.", data=phone_schema.dump(phone)), 200
